...
export default function App() {
  ...
  return <>
    ...
    <main id="main" className="wrap">
      ...
      <section className="hero" id="home"><div className="hero-content"><p className="eyebrow intro"><span/> SOFTWARE ENGINEER · FULL STACK & AI</p><h1>Engineering ideas.<br/>Creating <em>impact.</em></h1><p className="hero-description">Hi, I’m Asad. I turn complex problems into thoughtful digital experiences — from scalable web applications to intelligent, data-driven systems.</p><div className="hero-actions"><a className="button primary" href="#projects">Explore my work </a><a className="button secondary" href={resumeUrl} target="_blank" rel="noreferrer">View résumé </a></div><div className="hero-focus" aria-label="Areas of focus"><span className="hero-focus-label">FOCUSED ON</span><div><span>Full-stack Development</span><span>AI &amp; Machine Learning</span><span>Data Engineering</span></div></div></div><div className="hero-visual ascii-hero"><div className="visual-grid"/><AsciiScroll animated={motion && !intro} wireframe={wireframe}/><div className="visual-caption"><button className="wireframe-toggle" type="button" aria-pressed={wireframe} aria-controls="ascii-portrait" onClick={() => setWireframe(!wireframe)}>Wireframe {wireframe ? "on" : "off"}</button><button onClick={() => setMotion(!motion)} aria-pressed={motion}>{motion ? 'Pause' : 'Play'} motion</button></div></div></section>
      <details className="ascii-explainer" onToggle={() => window.dispatchEvent(new Event('resize'))}>
        <summary><span>Behind the ASCII</span><small>How the portrait comes together</small></summary>
        <div className="ascii-explainer-content">
          <div><span className="eyebrow">01 / CHARACTERS</span><h3>A moving cloud of code.</h3><p>Each character belongs to a particle on a rotating sphere. Its brightness changes with depth, giving the surface its shape.</p></div>
          <div><span className="eyebrow">02 / CONNECTION</span><h3>Every particle has a place.</h3><p>Hover or tap the sphere. Each particle travels to a fixed position and becomes a small tile of my photo. Together, the tiles assemble the portrait.</p></div>
          <div><span className="eyebrow">03 / UNDER THE SURFACE</span><h3>Follow the structure.</h3><p>Switch on Wireframe above to see each particle?s square boundary along the same path. The GPU draws the entire scene in one call per frame.</p></div>
        </div>
      </details>
      ...
      <section id="contact" className="contact section"><ContactSignature animated={motion && !intro}/><p className="eyebrow">HAVE SOMETHING IN MIND?</p><h2>Let’s build something<br/><em>worth putting out there.</em></h2><a className="button primary" href="mailto:asadbaig215@gmail.com">Start a conversation </a><a className="email-link" href="mailto:asadbaig215@gmail.com">asadbaig215@gmail.com</a></section>
    </main><footer className="wrap footer"><p>© {new Date().getFullYear()} Asad Baig · Crafted with intention.</p><div><a href="tel:+923365447781">Phone </a></div></footer>
  </div></>;
}
